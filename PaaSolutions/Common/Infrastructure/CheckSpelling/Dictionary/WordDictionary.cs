using Common.Infrastructure.CheckSpelling.Dictionary.Affix;
using Common.Infrastructure.CheckSpelling.Dictionary.Phonetic;
using System;
...
namespace Common.Infrastructure.CheckSpelling.Dictionary
{
    public class WordDictionary : System.ComponentModel.Component
    {
        ...
        public void Add(string word)
        {
            if (!string.IsNullOrEmpty(word))
            {
                ...
                {
                    MessageBox.Show("Item has already been added. Key in dictionary: '" + word + "'!", "Information", MessageBoxButtons.OK);
                }
            }
        }
        ...
        public void Initialize()
        {
            ...
            if (!Directory.Exists(_dictionaryFolder))
            {
                Directory.CreateDirectory(_dictionaryFolder);
                directoryExisted = false;
            }

            if (!File.Exists(dictionaryPath) || !directoryExisted)
            {
                using (WebClient webClient = new WebClient())
                {
                    webClient.DownloadFile(new Uri(_downloadUri), dictionaryPath);
                }
            }
        ...
        {
            if (!string.IsNullOrEmpty(word) && _userWords.Contains(word))
            {
